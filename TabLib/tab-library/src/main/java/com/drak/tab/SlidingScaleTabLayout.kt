...
import com.drak.tab.utils.ViewUtils.findBrotherView
import com.drak.tab.utils.ViewUtils.generateViewCacheBitmap
import java.util.*
...
class SlidingScaleTabLayout @JvmOverloads constructor(
    ...
    private fun obtainAttributes(
        ...
        val ta =
            context.obtainStyledAttributes(attrs, R.styleable.SlidingScaleTabLayout)
        mIndicatorStyle = ta.getInt(
            R.styleable.SlidingScaleTabLayout_tl_indicator_style,
            STYLE_NORMAL
        )
        ...
        mIndicatorWidth = ta.getDimension(
            ...
            dp2px(if (mIndicatorStyle == STYLE_TRIANGLE) 10F else -1F).toFloat()
        )
        ...
        textSelectSize =
            ta.getDimension(R.styleable.SlidingScaleTabLayout_tl_textSelectSize, textUnselectSize)
        ...
        mTextBold = ta.getInt(
            R.styleable.SlidingScaleTabLayout_tl_textBold,
            TEXT_BOLD_NONE
        )
        ...
        mTabPadding = ta.getDimension(
            ...
            if (mTabSpaceEqual || mTabWidth > 0) dp2px(0F).toFloat() else dp2px(9F).toFloat()
        )
        // 得到设置的上下间距和gravity
        mTabMarginTop =
        ...
        mTabHorizontalGravity = ta.getInt(
            R.styleable.SlidingScaleTabLayout_tl_tab_horizontal_gravity,
            CENTER
        )
        mTabVerticalGravity = ta.getInt(
            R.styleable.SlidingScaleTabLayout_tl_tab_vertical_gravity,
            CENTER
        )
        mTabMsgMarginTop =
        ...
        iTabScaleTransformer =
            TabScaleTransformer(this, textSelectSize, textUnselectSize, openDmg)
    }
    ...
    fun setViewPager(vp: ViewPager?, titles: Array<String?>?) {
        check(!(vp == null || vp.adapter == null)) { "ViewPager or ViewPager adapter can not be NULL !" }
        check(!(titles == null || titles.isEmpty())) { "Titles can not be EMPTY !" }
    ...
    private fun setTabLayoutParams(title: TextView) {
        var params =
            title.layoutParams as RelativeLayout.LayoutParams
        ...
        if (isDmgOpen) {
            val imageView = findBrotherView(
                title,
                R.id.tv_tab_title_dmg,
                3
            ) as ImageView?
                ?: return
    ...
    private fun addTab(
        ...
        tabView.setOnClickListener { view ->
            val position1 = mTabsContainer.indexOfChild(view)
            if (position1 != -1) {
    ...
    private fun updateTabStyles() {
        for (i in 0 until tabCount) {
            ...
            if (tv_tab_title != null) {
                if (i == 0) {
                    v.setPadding(mTabPadding.toInt() * 2, 0, mTabPadding.toInt(), 0)
                } else {
    ...
    private fun generateTitleDmg(
        ...
        val imageView =
            tabView.findViewById<ImageView>(R.id.tv_tab_title_dmg)
        imageView.setImageBitmap(generateViewCacheBitmap(textView))
        imageView.maxWidth = imageView.drawable.intrinsicWidth
    ...
    var transformers: List<IViewPagerTransformer>?
